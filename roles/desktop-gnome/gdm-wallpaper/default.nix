...
let
  ...
  cfg = config.mine.desktop-gnome.gdm-wallpaper;

  wallpaper = config.mine.desktop.system.wallpaper;
  wp = wallpaper.processed.blurred."${wallpaper.mode}";
  gdmCssOverlay =
...
in
{
  options.mine.desktop-gnome.gdm-wallpaper = {
    enable = mkEnableOption "GDM wallpaper";
  };
